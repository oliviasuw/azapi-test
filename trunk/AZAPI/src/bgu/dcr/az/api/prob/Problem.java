...
//    abstract public void setConstraintCost(int var1, int val1, int var2, int val2, int cost);
    ...
    /**
     * initialize the problem with a single domain that its values are 0..1-domainSize
     * @param type
    ...
    public int getConstraintCost(int owner, int x1, int v1) {
        return constraints.getConstraintCost(owner, x1, v1);
    }

    public int getConstraintCost(int owner, int x1, int v1, int x2, int v2) {
        return constraints.getConstraintCost(owner, x1, v1, x2, v2);
    }
    
    @Override
    public int getConstraintCost(int x1, int v1) {
        return constraints.getConstraintCost(x1, x1, v1);
    }

    @Override
    public int getConstraintCost(int x1, int v1, int x2, int v2) {
        return constraints.getConstraintCost(x1, x1, v1, x2, v2);
    }

    public int getConstraintCost(int owner, Assignment k) {
        return constraints.getConstraintCost(owner, k);
    }
    ...
    /**
     * calculate the cost of the given assignment - taking into consideration all the constraints that related to it, 
     * the method put the return value in an array of 2 [cost, number of constraints checked] that is given to the method as the parameter ans
     * @param owner
     * @param assignment
     * @return 
     */
    public void calculateCostAndCountCCs(int owner, Assignment assignment, int[] ans){
        constraints.calculateCost(owner, assignment, ans);
    }
