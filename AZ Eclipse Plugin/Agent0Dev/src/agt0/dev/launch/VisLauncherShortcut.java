...
public class VisLauncherShortcut implements ILaunchShortcut {
		...
		if (search.length == 0)
			...
			if (search[0] instanceof IJavaElement) {
				...
				try {
					...
					if (prj.getNature(AgentZeroProject.NATURE_ID) != null){
	...
	public void launch(IProject proj, String mode, Map<String, String> additionalAttributes) {
		...
		try {
			...
			ILaunchConfigurationType type = lman.getLaunchConfigurationType(VisLauncher.LAUNCH_CONFIGURATION_TYPE_ID);
			
			ILaunchConfigurationWorkingCopy copy = type.newInstance(proj, "a0v_" + proj.getName() + "_" + mode);//= lc.copy("test");
