...
public class DebugSelectionScreen extends javax.swing.JPanel implements TestView.DebugRequestListener {
    ...
    File problemsPath;
    List<ExperimentItem> badExperiments;
    ...
    void setProblemDir(File dir) {
        ...
        failList.setItems(Visual.adapt(badExperiments, new Visual.VisualGen() {
            ...
            public Visual gen(Object it) {
                try {
                    ExperimentItem ee = (ExperimentItem) it;
                    SimpleDateFormat format = new SimpleDateFormat("dd'/'MM'/'yy' 'HH':'mm':'ss");
                    return new Visual(it, format.format(ee.experimentSaveDate()), "", null);
                } catch (Exception ex) {
        ...
        failList.addSelectionListner(new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (!failList.getSelectedItems().isEmpty()) {
                    ...
                    CPExperiment selected = ((ExperimentItem) ((Visual) failList.getSelectedItems().get(0)).getItem()).experiment;
                    CPExperimentTest selectedTest = selected.getTestForExectution(selected.getSingleExecutionMode());

                    testView.setModel(selectedTest);
                    testView.addFailureData(selected);
    ...
    private void loadExperiments(File dir) {
        for (File f : dir.listFiles()) {
            try {
                badExperiments.add(new ExperimentItem(f));
            } catch (Exception ex) {
...
//        }
    ...
    private static class ExperimentItem {
        ...
        public ExperimentItem(File experimentFile) throws IOException, ConfigurationException {
            this.experiment = ConfigurationUtils.read(experimentFile).create();
            this.experimentFile = experimentFile;
        }

        Date experimentSaveDate() {
            return new Date(experimentFile.lastModified());
        }
    }
}
