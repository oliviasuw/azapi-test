 ...
 * @author bennyl
 */
public class StatisticsQueryScreen extends javax.swing.JPanel {
...
//    }
    ...
    private void jXHyperlink4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jXHyperlink4ActionPerformed
        ...
        try {
            ...
            if (!loc.getName().endsWith(".csv")) {
                loc = new File(loc.getAbsolutePath() + ".csv");
            }
            if (loc != null) {
    ...
    private void refreshTree() {
        try {
            tree.setModel(new GenericTreeModel(new DatabaseNode(StatisticsManagerImpl.getInstance().database().tables())));
    ...
    private void query(String q) {
        try {
            final Data data = StatisticsManagerImpl.getInstance().database().createQueryDatabase().query(q);

            resultList.setModel(new DataJTableModel(data));
...
//    }
    ...
    private class DatabaseNode extends GenericTreeModel.Node implements SqlRepresenter {
        ...
        public DatabaseNode(Map<String, TableMetadata> metadata) {
            super("Statistics Database", null);
            children = new LinkedList<DatabaseTableNode>();
            for (TableMetadata t : metadata.values()) {
                children.add(new DatabaseTableNode(t, this));
    ...
    private class DatabaseTableNode extends GenericTreeModel.Node implements SqlRepresenter {
        ...
        public DatabaseTableNode(TableMetadata table, Node parent) {
            super(table.name(), parent);
            this.name = table.name();

            children = new LinkedList<DatabaseFieldNode>();
            for (FieldMetadata f : table.fields()) {
                children.add(new DatabaseFieldNode(f.name(), f.type().getSimpleName(), this));
            }
