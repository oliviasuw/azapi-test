...
import bgu.csp.az.api.tools.TimeStamp.OldMessageHandlingPolicy;
...
public class AFBAgent extends SimpleAgent {
	...
	public void start() {
		...
		//ts.register(this, OldMessageHandlingPolicy.DISCARD_OLD_MESSAGES);
	...
	private void assignCPA() {
		...
		//log("last assigned value: " + lastAssignedValue + " in cpa: " + cpa);
		cpa.unassign(this.getId());
		
		for (int i = lastAssignedValue + 1; i < this.getDomainSize(); i++) {
			//log("testing assignment of val: " + i + ", cost of cpa =" + costOf(cpa) +  ", calcfc = " + calcFv(i, cpa) + ", while b=" +b);
		...
		if (v == -1){
			...
			//log("assigning " + v);
			if (cpa.getNumberOfAssignedVariables() == this.getProblem().getNumberOfVariables()){
	...
	protected SimpleMessage beforeMessageProcessing(SimpleMessage msg) {
		...
		if (hisTimeStamp.compare(timeStamp, this) >= 0){
			//log("updating to time stamp " + hisTimeStamp + " from " + timeStamp);
			timeStamp.set(hisTimeStamp);
		}else {
			//log("dumping message: " + msg + " time stamp: " + hisTimeStamp + " older then mine: " + timeStamp);
			return null;
		}
		
		//log("Agent Got Message: " + msg);
	...
	public void handleFBESTIMATE(int estimate, Assignment pa, int aj) {
		...
		if (costOf(cpa) + estimatesSum() >= b) {
			...
			//log("dumping estimation cause costOf(cpa) + estimatesSum() < b: " + (costOf(cpa) + estimatesSum())  + "<" + b);
		}
	}
