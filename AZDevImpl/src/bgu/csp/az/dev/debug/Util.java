...
public class Util implements DeepCopyable{
	...
	public class AgentUtil implements Comparable<AgentUtil>, DeepCopyable{
		...
		public Object deepCopy() {
			...
			ans.agentValues = new LinkedList<Util.AgentValue>();
			ans.util = this.util;
	...
	public Util(ImuteableProblem problem, int agentId, int domain) {
		this.problem = problem;
		this.agent = agentId;
		this.domainSize = domain;
		agentUtils = new LinkedList<AgentUtil>();
		childrenUtils = new LinkedList<Util>();
		relevantAncestors = new LinkedList<Integer>();
	}
	
	
	public void addChildUtil(Util util) {
	...
	private void computeInnerNodeUtil(DPOPAgent da) {
		for (Util u : childrenUtils) { // add children's ancestors
	...
	public Object deepCopy() {
		...
		Util ans = new Util(this.problem, this.agent, this.domainSize);
		
		ans.agentUtils = new LinkedList<AgentUtil>();
		ans.childrenUtils = new LinkedList<Util>();
		ans.relevantAncestors = new LinkedList<Integer>();
		
		for (AgentUtil au : this.agentUtils){
			ans.agentUtils.add((AgentUtil) au.deepCopy());
		}
		for (Util u : this.childrenUtils){
			ans.childrenUtils.add((Util) u.deepCopy());
		}
