...
public class LogScreen extends javax.swing.JPanel implements bgu.dcr.az.dcr.api.modules.Logger {
    ...
    public LogScreen() {
        ...
        pw = new UIPoke(this::updateLogView, 100, UIPoke.UIType.SWING);
    ...
    private void updateLogView() {
        boolean lastLine = TextUtils.getLines(output) == TextUtils.getLineAtCaret(output);

        LogLine line = null;
        while ((line = pendingLogLines.poll()) != null) {
            doc.addLog(line.line, line.teller, Level.INFO);
        }
        try {
        ...
        if (lastLine) {
            TextUtils.gotoStartOfLine(output, TextUtils.getLines(output));
        }
    }
...
private void nextMatchActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_nextMatchActionPerformed
    ...
    public void log(String logger, String msg) {
        pendingLogLines.add(new LogLine(logger, msg));
        pw.poke();
    }

    @Override
    public void initialize(Execution<CPData> ex) throws InitializationException {
        log("AZ-SYS", "Starting execution of " + ex.data().getAlgorithm().getInstanceName());
    }
    ...
    private static class LogLine {
        ...
        public LogLine(String line, String teller) {
            this.line = line;
            this.teller = teller;
        }
