 ...
 * @author BLutati
 */
public class FormNodeView extends javax.swing.JPanel implements NodeView {
    public static final String ATT_COLOR_LABELS = "FormNodeView COLOR LABELS";
    public static final String ATT_CAPTION = "FormNodeView CAPTION";
    public static final String ATT_HEIGHT_EXPAND_PRECANTEGE = "FormNodeView HEIGHT EXPAND PRECANTEGE";
...
//        lbl.setVerticalAlignment(SwingConstants.TOP);
    ...
    private void addValueView(Node n, JComponent defaultView, int columns) {
        GridBagConstraints cons = new GridBagConstraints();
        if (columns == 3) {
            cons.gridx = 0;
        } else {
        ...
        int heightExpandPrecentage = n.getIntegerAtt(ATT_HEIGHT_EXPAND_PRECANTEGE, -1);

        if (heightExpandPrecentage > 0) {
            cons.weighty = 100.0 / heightExpandPrecentage;
            heightTaken = true;
        }
    ...
    public void setNode(Node node) {
        ...
        List<Node> children = this.node.childrenList();

        Collections.sort(children, new Comparator<Node>() {

            public int compare(Node o1, Node o2) {
                return o1.getIntegerAtt(Att.SEQUANCE_NUMBER, 0) - o2.getIntegerAtt(Att.SEQUANCE_NUMBER, 0);
            }
        });

        for (Node c : children) {
            if (!exclude.contains(c.getStringAtt(Att.ID))) {
                String caption = (c.hasAtt(ATT_CAPTION)? c.getStringAtt(ATT_CAPTION): c.getStringAtt(Att.ID));
                String[] nameLR = caption.split("\\$\\$");

                if (nameLR[0].startsWith("#")) {
                    addValueView(c, c.getView(), 3);
                } else {
                    addLabel(nameLR[0], SwingDSL.resIcon(c.getStringAtt(Att.ICON)));
                    addValueView(c, c.getView(), 3 - nameLR.length);
                    if (nameLR.length > 1) {
    ...
    private void style(Node node) {
        if (!node.getBooleanAtt(ATT_COLOR_LABELS, true)){
            colorLabels = false;
        }
    }
}
