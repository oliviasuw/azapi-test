...
//        for (int i = 0; i < n; i++) {
//            for (int j = 0; j < edgesToConnect[i]; j++) {
...
//        }
          ...
          while (v.size()>2){ Integer v1 = v.get(rand.nextInt(v.size()));
          v.remove(v1); Integer v2 = v.get(rand.nextInt(v.size()));
          v.remove(v2);
         
          buildConstraint(v1, v2, p, rand);
         
          edgesToConnect[v1]--; edgesToConnect[v2]--;
         
          if (edgesToConnect[v1]>0) v.add(v1); if (edgesToConnect[v2]>0)
          v.add(v2); } if (!v.isEmpty()){ System.err.println("NOTE THAT PROBLEM IS NOT REALLY REGULAR... :)");
    	}
