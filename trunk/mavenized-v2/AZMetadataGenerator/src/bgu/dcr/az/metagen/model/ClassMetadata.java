...
public class ClassMetadata extends Metadata<TypeElement> {
    ...
    public ClassMetadata getSuper() {
        if (getFQN().equals("java.lang.Object")) {
            return null;
        }
    ...
    /**
     * search for the first super class in the chain to object that has the
     * given annotation
     *
     * @param annotationFQN
     * @return
    ...
    public ClassMetadata lookupSuperClassWithAnnotation(String annotationFQN) {
        ...
        while (s != null) {
            if (s.hasAnnotation(annotationFQN)) {
                return s;
            }
            s = s.getSuper();
        }
    ...
    public boolean hasMethod(String fullSignature) {
        for (MethodMetadata m : getMethods()) {
            if (fullSignature.equals(m.getFullSignature())) {
                return true;
            }
        }
    ...
    /**
     *
     * @return list of property names in this class (property = one that has
     * getter..)
     */
