...
public class NoGoodStore {
	...
	public NoGoodStore(int ownerId) {
		ngStore = new HashMap<Integer, HashSet<NoGood>>();
		ID = ownerId;
	}
	...
	/**
	 * Check the agentview against existing nogood store
	 * if a nogood's lhs contains a var-val pair which is 
	 * inconsistent with the agentview we remove it.
	 * Using iterators to avoid concurrent modification errors
	 * @param av
	 */
	...
	/**
	 * check if an agent contains a valid explanation for the removal of value from
	 * the current domain.
	 * Note: an empty (non null) explanation is also valid! 
	 * @param value
	 * @return false if value has no NoGood explanation for the removal of "value" and true otherwise 
	 */
	...
	/**
	 * Generate a new nogood composed of all agents currently existing in the ngstore
	 * The rhs is the highest agent in the set (this is the agent who will receive this
	 * new nogood) and the lhs is composed of all remanining agents.
	 * @return
	 */
	public NoGood solve(){
		NoGood res = new NoGood();
		TreeMap<Integer,Integer> agents = new TreeMap<Integer, Integer>();
	...
	public Collection<Integer> eliminatedVals(){
		return ngStore.keySet();
	}
	
	@Override
	public String toString() {
		//return "NG-store contains EE for "+ngStore.keySet().toString();
		return "NG-store: "+ngStore.toString();
	}
}
