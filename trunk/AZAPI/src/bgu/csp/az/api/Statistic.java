...
public class Statistic implements Serializable {
    ...
    public Statistic() {
        ...
        childs = new HashMap<String, Statistic>();
    }
     ...
     */
    public Map<String, Statistic> getChilds() {
        return childs;
    }

    /**
     * @return the sum of all childs
     */
    public long sumChilds(){
        long sum = 0;
        for (Entry<String, Statistic> c : childs.entrySet()){
    ...
    /**
     * @return the avarage of all childs
     */
    public long avgChilds(){
        return sumChilds() / childs.size();
    }
    
    /**
     * @return the maximum value from all childs
     */
    public long maxChilds(){
        long max = Long.MIN_VALUE;

        for (Entry<String, Statistic> c : childs.entrySet()){
            max = Math.max(max, c.getValue().getValue());
        }
    ...
    /**
     * 
     * @return the minimum value from all childs
     */
    public long minChilds(){
        long min = Long.MAX_VALUE;

        for (Entry<String, Statistic> c : childs.entrySet()){
            min = Math.min(min, c.getValue().getValue());
        }
