...
public class Page implements Model.ParameterChangeListener {
    ...
    public List<ParamModel> getParamsWithRole(String role) {
        LinkedList<ParamModel> ret = new LinkedList<ParamModel>();
        for (ParamModel p : getParams()) {
    ...
    private ParamModel generateParam(Param dano, Field field, Model m) {
        ...
        final Method selc = readSelectionChangedCallback(model, dano.name());
        if (selc != null) {
            pmod.addSelectionListner(new ParamModel.Listener() {

                public void onChange(ParamModel source, Object newValue, Object deltaHint) {
                    try {
                        selc.invoke(model, source, newValue);
                    } catch (Exception ex) {
                        Logger.getLogger(Page.class.getName()).log(Level.SEVERE, "On Method With WhenSelectionChanged Annotation: " + model.getClass().getSimpleName() + "." + selc.getName() + ""
                                + "\ntrying to send: source:" + source.getClass() + ", newValue: " + newValue.getClass(), ex);
                    }
                }
            });
        }
