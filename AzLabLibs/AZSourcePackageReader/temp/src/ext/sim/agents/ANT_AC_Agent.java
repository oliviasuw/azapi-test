...
@Algorithm(name="ANT+AC", useIdleDetector=true)
public class ANT_AC_Agent extends SimpleAgent {
    ...
    @Override
    public void start() {
    	ac = new NestableACNash();
		ac.calculate(this).andWhenDoneDo(new Continuation() {
    ...
    private void updateAgentView(Integer agent, Integer value){
    	...
    	currentDomain = new HashSet<Integer>(pStarDomain);
    ...
    private boolean coherent(NoGood ng, HashSet<Integer> agentSet) {
		...
		for (Integer var : ng.getAgents())
			if (!agentView.isAssigned(var))
				agentView.assign(var, ng.valueOf(var));
		return true;
	}
